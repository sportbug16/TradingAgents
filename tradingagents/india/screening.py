...
def merge_provider_outputs(output_dir: str | Path, providers: list[str]) -> dict[str, Any]:
    ...
    for provider in providers:
        ...
        provider_results[provider] = (
            ...
            if path.exists()
            else provider_result(provider, "missing", [], reason="provider output not found")
        )
    merged = {"run_at": _now(), "providers": provider_results}
    _write(root / "merged.json", merged)
    return merged


def write_provider_status(output_dir: str | Path, provider: str, status: str, reason: str | None = None) -> dict[str, Any]:
...
def _screen_packet(client, provider: str, model: str, packet: dict[str, Any]) -> IndiaScreenDecision:
    try:
        response = client.invoke(_screen_prompt(packet))
...
def _screen_prompt(packet: dict[str, Any]) -> str:
    ...
    return (
        ...
        "Buy, Overweight, Hold, Underweight, Sell. confidence is 0.0 to 1.0.\n\n"
